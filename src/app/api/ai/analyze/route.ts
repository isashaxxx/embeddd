    ...
    body: JSON.stringify({
      ...
      input: [{
        ...
        content: [
          {
            type: 'input_text',
            text: `Ты умный куратор визуальной библиотеки. Проанализируй не только предмет на изображении, но и его стиль, назначение, настроение и визуальные признаки. Дай короткое конкретное название на русском (2-6 слов), полезное описание одним предложением и ровно три устойчивых варианта коллекции: сначала наиболее подходящая существующая коллекция, затем более узкая тема и более широкая тема. Не создавай коллекцию для единичной мелкой детали. Если подходит существующая коллекция, используй её точное название первым вариантом. Существующие коллекции: ${existing.length ? existing.join(', ') : 'нет'}.`,
          },
